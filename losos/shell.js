...
async function loadData() {
  ...
  // Also check #data div (inline data pattern used by API-driven apps)
  var dataDiv = document.getElementById('data')
  if (dataDiv && dataDiv.textContent.trim() && !rawData) {
    try {
      var parsed = JSON.parse(dataDiv.textContent)
      store.load(parsed)
      rawData = parsed
    } catch (err) {
      console.warn('[losos] Failed to parse #data div:', err)
    }
  }
...
function renderTabs(panes, container, subject, store, rawData, opts) {
  ...
  var tabColor = opts.tabColor || 'rgba(0,0,0,0.4)'
  var tabActiveColor = opts.tabActiveColor || 'rgba(0,0,0,0.85)'

  const tabBar = document.createElement('div')
  tabBar.id = 'pane-tabs'
  tabBar.style.cssText = 'display:flex;gap:0;border-bottom:1px solid rgba(0,0,0,0.08);overflow-x:auto;max-width:' + maxWidth + ';margin:0 auto'
  ...
  const selectPane = (pane, tab) => {
    ...
    tab.style.borderBottomColor = opts.accentColor || '#6366f1'
